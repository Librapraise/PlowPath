...
export async function sendPaymentReminder(req: Request, res: Response): Promise<void> {
  ...
  const { rows } = await query<{
    ...
    `SELECT name, address, email, outstanding_balance, payment_status
       FROM customers
  ...
  if (!customer.email) {
    throw HttpError.badRequest(`Customer "${customer.name}" does not have a registered email address.`);
  }

  const balance = Number(customer.outstanding_balance || 0);
  if (balance <= 0) {
    throw HttpError.badRequest(`Customer "${customer.name}" has no outstanding balance.`);
  }

  const subject = 'OVERDUE ACCOUNT BALANCE INVOICE WARNING';
